...
@pytest.mark.parametrize("kv_len", [54, 231, 511, 788, 944])
@pytest.mark.parametrize("qo_len", [17, 47, 127, 577, 711])
@pytest.mark.parametrize("stride", [1])
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("num_kv_heads", [4])
@pytest.mark.parametrize("num_qo_heads", [28])
...
def test_pod_with_paged_kv_cache(
    ...
    # print(batch_size, q.shape, kv_data.shape)
    # o_ref, _ = attention_ref(batch_size, q, kv_data[:, 0], kv_data[:, 1], causal, 1. / math.sqrt(head_dim))

    o_ref = torch.ones_like(o_fa2)

    torch.testing.assert_close(o_fa2, o, rtol=1e-3, atol=1e-3)


if __name__ == "__main__":
    test_pod_with_paged_kv_cache(
        1, 788, 577, 1, 1, 4, 28, 64, True, "NHD", "NONE", False, 0.0, False, True
    )
