...
struct SparseCollectiveMainloop {
  ...
  CUTLASS_DEVICE void load(Params const& mainloop_params, MainloopPipeline pipeline_k,
    ...
    // K and V may have different strides (e.g., non-contiguous KV-cache)
    ...
    // Separate buffers for K and V to support non-contiguous KV-cache (different strides)
    int64_t my_k_offset[2];  // Rolling buffer: page_idx * k_page_stride + entry_idx * k_stride_n
    int64_t my_v_offset[2];  // Rolling buffer: page_idx * v_page_stride + entry_idx * v_stride_n
    ...
    // Prefetch: compute page_idx * page_stride + entry_idx * stride_n for both K and V
    ...
    auto prefetch_kv_offset = [&](int kv_tile_idx, bool use_predicate) {
      ...
      if (valid_read) {
        ...
        my_k_offset[parity] = page_idx * k_page_stride + entry_idx * k_stride_n;
        my_v_offset[parity] = page_idx * v_page_stride + entry_idx * v_stride_n;
      } else {
        my_k_offset[parity] = 0;
        my_v_offset[parity] = 0;
      }
    };

    // Unified helper lambda to load K or V with pre-computed offsets
    // offset_buf: pointer to the per-thread offset buffer (my_k_offset or my_v_offset)
    // Uses parity to select buffer slot, caller must toggle parity after load
    auto load_kv_with_gather = [&](auto&& tXsX, auto&& tXcX, DTypeKV* base_ptr, int64_t* offset_buf,
      ...
      for (int i = 0; i < size(dst); ++i) {
        ...
        int64_t base_offset = __shfl_sync(FULL_MASK, offset_buf[parity], src_thread);
    ...
    // load last k-tile
    // parity=0: prefetch kv_tile_idx -> my_k_offset[0], my_v_offset[0]
    {
      prefetch_kv_offset(kv_tile_idx, true);
      pipeline_k.producer_acquire(smem_pipe_write_k);
      load_kv_with_gather(tKsK, tKcK, K_ptr_base, my_k_offset, kv_tile_idx,
                          smem_pipe_write_k.index(), true);
    ...
    if (kv_tile_idx == swa_begin_kv_tile_idx) {
      ...
      load_kv_with_gather(tVsV, tVcV, V_ptr_base, my_v_offset, kv_tile_idx,
                          smem_pipe_write_v.index(), true);
      pipeline_v.producer_commit(smem_pipe_write_v, cutlass::arch::cpasync_barrier_arrive);
      ++smem_pipe_write_v;
    } else {
      // load second last k-tile and last v-tile
      // parity=0: kv_tile_idx is in my_k_offset[0], my_v_offset[0]
      // Now prefetch kv_tile_k into my_k_offset[1], my_v_offset[1]
      int kv_tile_k = kv_tile_idx_decrement(kv_tile_idx);
      parity ^= 1;  // parity=1
      prefetch_kv_offset(kv_tile_k, false);
      pipeline_k.producer_acquire(smem_pipe_write_k);
      load_kv_with_gather(tKsK, tKcK, K_ptr_base, my_k_offset, kv_tile_k, smem_pipe_write_k.index(),
                          false);
      pipeline_k.producer_commit(smem_pipe_write_k, cutlass::arch::cpasync_barrier_arrive);
      ++smem_pipe_write_k;

      // Load V for kv_tile_idx using my_v_offset[0]
      parity ^= 1;  // parity=0
      pipeline_v.producer_acquire(smem_pipe_write_v);
      load_kv_with_gather(tVsV, tVcV, V_ptr_base, my_v_offset, kv_tile_idx,
                          smem_pipe_write_v.index(), true);
      ...
      ++smem_pipe_write_v;
      // Now kv_tile_idx == kv_tile_k, and its offset is in my_k_offset[1], my_v_offset[1]
...
#pragma unroll 2
      for (; kv_tile_idx > swa_begin_kv_tile_idx;
        ...
        prefetch_kv_offset(kv_tile_k, false);
        pipeline_k.producer_acquire(smem_pipe_write_k);
        load_kv_with_gather(tKsK, tKcK, K_ptr_base, my_k_offset, kv_tile_k,
                            smem_pipe_write_k.index(), false);
        pipeline_k.producer_commit(smem_pipe_write_k, cutlass::arch::cpasync_barrier_arrive);
        ++smem_pipe_write_k;
      ...
      // load first v tile (tile 0)
      {
        ...
        pipeline_v.producer_acquire(smem_pipe_write_v);
        load_kv_with_gather(tVsV, tVcV, V_ptr_base, my_v_offset, 0, smem_pipe_write_v.index(),
                            false);
        pipeline_v.producer_commit(smem_pipe_write_v, cutlass::arch::cpasync_barrier_arrive);
        ++smem_pipe_write_v;
      }
