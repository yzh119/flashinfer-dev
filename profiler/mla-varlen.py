...
def profile_deepseek_mla_decode(
    seq_lens, append_lens, num_heads, profiler_buffer_size, backend
):
    head_dim_ckv = 512
    head_dim_kpe = 64
    page_size = 1
    ...
    ckv = torch.randn(sum(seq_lens), 1, head_dim_ckv, dtype=torch.half, device="cuda")
    kpe = torch.zeros(sum(seq_lens), 1, head_dim_kpe, dtype=torch.half, device="cuda")
    ...
    export_to_perfetto_trace(
        ...
        f"mla-{backend}-{'-'.join(map(str,seq_lens))}-{'-'.join(map(str,append_lens))}-{num_heads}.perfetto-trace",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seq-lens", type=int, nargs="+", default=[4641, 45118, 1730, 1696]
    )
    parser.add_argument("--append-lens", type=int, nargs="+", default=[4, 4, 4, 4])
